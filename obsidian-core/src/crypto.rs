...
impl MasterKey {
    ...
    /// Те же байты, но наружу.
    ///
    /// Нужны там, где ключ уходит не в наше запечатывание, а в чужое, —
    /// например в DPAPI на Windows, который берёт произвольную «энтропию» и
    /// без неё расшифровывает файл кому угодно из-под той же учётной записи.
