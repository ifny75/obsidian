 ...
 *
 * Один источник на выбор сервисов и на меню: раньше номера версий стояли по
 * месту и разъехались, и с сервисами повторять эту ошибку незачем.
 *
 * `ready` честно отделяет работающее от задуманного. Мессенджер выпущен,
 * почта и VPN пока прототипы — и подписаны как прототипы. Обещать в интерфейсе
 * то, чего нет, дороже, чем промолчать: человек попробует и не найдёт.
 */
export type Service = {
  id: 'messenger' | 'mail' | 'vpn';
  name: string;
  /** Временная метка вместо логотипа: их ещё не нарисовали. */
  letter: string;
  badge: string;
  tagline: string;
  about: string;
