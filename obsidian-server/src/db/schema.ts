 ...
 */
export const SCHEMA = `
-- handle — мёртвое поле, и заполнять его нельзя.
--
-- Это было человекочитаемое имя, которое клиент присылал при регистрации, и
-- сервер хранил его открытым текстом рядом с ключом личности: готовая связка
-- «как человека зовут ↔ кто он криптографически». Читал его при этом никто —
-- поиск людей давно живёт в usernames, где лежат хеши.
--
-- Столбец не удалён только потому, что он UNIQUE: SQLite не умеет ALTER TABLE
-- DROP COLUMN для проиндексированных столбцов, а пересобирать таблицу с
-- личностями ради пустого поля — риск больше пользы. Значения из старых баз
-- вычищаются миграцией, новые сюда не попадают.
...
-- last_seen округляется до часа, и это не небрежность.
--
-- С точностью до миллисекунды это готовый график жизни человека: когда встал,
-- когда лёг, когда был в дороге. Серверу же он нужен ровно для одного — понять,
-- какое из устройств отвечало последним. Для этого хватает часа.
...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  --
  -- Клиент, у которого правило profile_avatar не «все», присылает их
  -- запечатанными в decor, а эти два поля очищает: украшения — это то же
  -- самое, что аватар, только мельче, и прятать одно, оставив другое, было бы
  -- притворством. Что внутри блоба, сервер не знает и проверить не может —
  -- проверяет теперь клиент получателя, который их и рисует.
  emblem      TEXT,
  color       TEXT,
  decor       BLOB,
...
-- Автор и подпись хранятся рядом с постом и сервером не проверяются.
--
-- Право писать сервер решает по-прежнему сам, но подпись — это другое: она
-- отвечает на вопрос «кто это написал», а не «кому можно». Раньше на него
-- отвечал тоже сервер, и захвативший его писал бы от имени любого канала и
-- правил историю задним числом, а читатель не отличил бы. Теперь подписывает
-- автор своим ключом личности, проверяет читатель, а мы только храним.
--
-- Отсюда и id с временем в кадре: подписать то, что сервер придумает после,
-- автор не может. Время он называет сам, а сервер лишь не пускает откровенную
-- ложь — окно в четверть часа.
