...
fn main() {
    ...
    /*
      И заодно: подделка обязана не проходить, иначе проверка выше ничего не
      значит. Портим гарантированно — приписываем пробел, — а не заменой
      подстроки: первая версия этого примера искала "0.1.0", и на манифесте без
      такой строки «подделка» совпадала с оригиналом и бодро проходила проверку.
      Тест, который врёт в утешительную сторону, хуже отсутствующего.
    */
    let tampered = format!("{manifest} ");
    assert_ne!(tampered, manifest, "подделка обязана отличаться от оригинала");
